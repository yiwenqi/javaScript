...
//                             注意延迟脚本会按照他们在文档中的出现顺序执行，而异步脚本在他们加载后执行，这意味着他可能会时无序的执行
        ...
        1.6.安全性
                1.6.1.JavaScript不能做什么
                        1.第一道防线:JavaScript不支持某些功能，例如不支持JavaScript代码没有写入和删除客户计算机任意文件的权限。这意味着javaScript程序不能
                        植入病毒和进行数据删除。

                        2.第二道防线:对自己支持的某些功能进行限制
                                        eg:
                                                ·javaScript程序可以打开一个新的窗口，但是为了防止广告商滥用这一功能，使得只有在响应鼠标单击事件
                                                的时候才能使用这样的功能。

                                                ·JavaScript能够关闭窗口，但不允许不经过用户同意就关闭窗口。

                                                ·脚本不能读取不同服务器上的文档内容（如密码），除非这个就是包含改脚本的文档。及一个文档上可以出现来自两个不同服务器
                                                上的脚本，但是不同的文档上的脚本之间不能读取对方的文档内容。

                                                ·HTML FileUpload元素的value属性是只读。如果可以操作这个属性那么脚本就能上传指定的文件到服务器，如隐私文件和密码文件
                                                这些比较重要的文件。

                                        tip:这里并没有给出全部的安全限制，不同的浏览器有不同的安全策略。


                1.6.2.同源策略
                        同源策略是对JavaScript代码能够操作那些web内容的一条完成的安全限制。

                        当web页面使用多个<iframe>元素或者打开其它浏览器窗口的时候，这个策略则会发挥作用。在这种情况下，同源策略负责管理窗口或窗口
                        中的JavaScript代码以及和其它窗口或帧的交互。具体来说，脚本只读取和其所属文档相同来源的窗口和文档的属性。

                        文档的来源包含协议，主机，以及载入文档的URL端口，从不同服务器的文档来源不同。注意使用http:协议载入的文档和用https:协议
                        载入的文档来源不同，即使他们来自同一服务器。

                        ※注意:脚本本身的来源与同源策略并没有关系，相关的是脚本所嵌套的文档的来源。如:一个来自主机A的脚本，被一个主机B的文档使用src引入A中的脚本
                        那么脚本A的来源是主机B,如果脚本打开一个新的窗口并载入来自B的另一个文档，那么A脚本对它具有相同的访问权限。但是当脚本打开第三个窗口并
                        载入一个来自C的文档（或者A）那么，这个时候同源策略就会发挥作用。阻止脚本访问文档。

                        对于防止脚本窃取私有的信息来说，同源策略是必须的。他防止了一个恶意的脚本打开一个新的窗口并欺骗用户进入并使用这个窗口在内网上浏览文件。
                        恶意脚本能够读取窗口的内容将其发回自己的服务器，同源策略阻止了这类事情。


                      不严格的同源策略。
                        当home.example.com上的脚本想要合法的访问来自order.example.com上的文档内容时可以使用document对象的domain属性，domain属性
                        存放的是载入文档的服务器主机名。那么你可以将两个载入文档的domain都设置为example.com

                        不严格的同源策略第二种技术已经标准化为:跨域资源共享。
                                这个标准的草案用新的"Origin:"请求头和新的Access-Control-Allow-Origin响应头来扩展HTTP。它允许服务器用头消息显式的列出源，
                                或者使用通配符来匹配所有的源头并允许任何地址请求文件。

                1.6.4.跨站脚本（xss）
                        跨站脚本是攻击者向目标Web站点注入html标签或者脚本。防止XSS攻击是服务器端web开发者的一项基本工作。然而，客户端JavaScript程序员也必须意识到
                        或者能够预防跨站脚本
